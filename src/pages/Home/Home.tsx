...
const Home = () => {
    ...
    // order === ordering - если у нас уже быыл выбран порядок, то он отменяется
    // в противном случае - он применяется и сортируется
...
// HW11 Добавить сортировку на главную страницу приложения 
// Нужно сделать 2 кнопки на домашней странице Title / Date
// При нажали на Title - сортировка по Title
// При нажатии на Date - сортировка по Date
// Order (прокинуть дополнительно в объекте - доп параметр ordering из запроса get blog/posts). Именно когда мы получаем посты => закинуть в депенденси array
// => ordering может быть по Title и Date (orderingByTitle и orderingByDate)
// Ставим primary и при нажатии на одну кнопочку - сетается в stateDate,  при нажатии на другую - stateTitle
// В getSearchedPost добавить новые параменты в скобочках (после search, offset) и дальше прокинуть в сам запрос
// Фильтрация происходит на стороне backend 
// => Нужен только enum для кнопочек => будет date и title => при нажатии на одну кнопочку - сетается в stateDate,  при нажатии на другую - stateTitle и вот эту переменную ordering прокивыдаваем в action

// step 1 HW11 (сортировка Title и Date по кнопке)
// step 1 В api в запросе get blog/posts добавляем новый параметр ordering
// step 2 в postSlice проверяем наш getPostWorker. Нужно проверить payload так как добавился параметр ordering => в GetPostsPayload добавляем ordering 
// step 3 в postSaga работаем с воркером getPostsWorker. Прокидываем через него ordering.
// step 4 на страничке Home работаем с кнопочками  Title и Date. Так как меняется их состояние  - используем useState для ordering
// step 5 так как у нас 2 состояния кнопочек  для сортировки то можем сделать enum Order в @types => будет date и title
//  step 6 создаем функцию onSortButtonClick, которая будет отвечать за нашу фильтрацию по кнопке . В нее запихиваем ранее созданный enum Order
// step 7 прописываем верстку для кнопочек в return.  Используем компонент Button
// step 8 в useEffect передаем ordering в качестве зависимости
// NB!!! выскакмвала ошибка 403 на кноаочку фильтр. Причина - наличие serch? в запросе
// а в воркере getPostsWorker мы его не передали
// решение: или '', или засунуть в объект вместе с serch (деструктурировать объектом)
// нельзя просто поменять местами в запросе, так как важен порядок, в котором принимает функция параметры
